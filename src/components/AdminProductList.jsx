...
function AdminProductList() {
  ...
  function handleOnSortChnage(e, option) {
    const sorting = { _sort: option.sort };
    setSort(sorting);
  }
  function handleOnFilterChange(e, section, option) {
    ...
    if (!newFilter[section]) {
      newFilter[section] = []; // Initialize the array if it doesn't exist
    }

    if (e.target.checked) {
      newFilter[section].push(option);
    } else {
      newFilter[section] = newFilter[section].filter((x) => x !== option);
    }
    setFilter(newFilter);
  }

  const nextPage = () => {
    if (page !== Math.ceil(allItemsCount / PER_PAGE_ITEM_LIMIT)) {
      setPage(page + 1);
    }
  };
        ...
        <div>
          ...
          <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
              ...
              <SortMenu
                handleOnSortChnage={handleOnSortChnage}
...
function SortMenu({ sortOptions, setMobileFiltersOpen, handleOnSortChnage }) {
  return (
            ...
            <div className="py-1">
              {sortOptions.map((option) => (
                <Menu.Item key={option.name}>
                  {({ active }) => (
                    <p
                      onClick={(e) => handleOnSortChnage(e, option)}
                  ...
                  {section.options.map((option, optionIdx) => (
                    <div key={option.value} className="flex items-center">
                      <input
                        ...
                        className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                        onChange={(e) => {
                          handleOnFilterChange(e, section.id, option.value);
                        }}
                      />
                      <label
